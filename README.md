...
5. **Observability (ELK)** — apply `argocd/elastic-application.yaml` to
   `openshift-gitops`. It installs the ECK operator (OLM, scoped to the
   `elastic` namespace) plus a single-node Elasticsearch, Kibana behind an
   edge-TLS Route, and an APM Server. The backend exports OpenTelemetry
   traces (otelgin per-request spans + a span per statement-processing job)
   and all `budgeit_*`/runtime metrics (via the OTel Prometheus bridge) over
   OTLP/HTTP to the APM server, which stores them in Elasticsearch as
   `traces-apm*`/`metrics-apm*`. A PostSync hook imports the *Budge-it
   Observability* dashboard into Kibana, and the frontend header links to it
   (`KIBANA_URL`/`KIBANA_DASHBOARD_URL` in `deploy/base/configmap.yaml` —
   update the hosts for your cluster domain, along with
   `server.publicBaseUrl` in `deploy/elastic/kibana.yaml`). Rotate the
   shared APM secret token: the same value goes in
   `budgeit-apm-secret-token` (elastic namespace) and
   `budgeit-secrets/ELASTIC_APM_SECRET_TOKEN` (budge-it namespace). Log in
   to Kibana as `elastic` with
   `oc -n elastic get secret budgeit-es-elastic-user -o jsonpath='{.data.elastic}' | base64 -d`.
